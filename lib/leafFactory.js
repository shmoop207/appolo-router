...
class LeafFactory {
    static createLeaf(part, parts, index) {
        if (util_1.Util.isRegex(part)) {
            return new regexLeaf_1.RegexLeaf(part, parts, index);
        }
        else if (util_1.Util.isParam(part)) {
            return new paramLeaf_1.ParamLeaf(part);
        }
        else {
            return new staticLeaf_1.StaticLeaf(part);
        }
    }
}
